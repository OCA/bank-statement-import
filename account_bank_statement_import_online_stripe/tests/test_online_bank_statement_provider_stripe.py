...
_module_ns = "odoo.addons.account_bank_statement_import_online_stripe"
_provider_class = (
    _module_ns
    + ".models.online_bank_statement_provider_stripe"
    + ".OnlineBankStatementProviderStripe"
)


class TestOnlineBankStatementProviderStripe(common.TransactionCase):
    def setUp(self):
        super().setUp()
    ...
    def test_obtain_statement_data_success(self):
        ...
        with patch(
            _provider_class + "._stripe_api_get_all",
            return_value=iter(mock_stripe_data),
        ) as mock_get_all:
            ...
            self.assertEqual(
                statement_data[0]["name"], "stripe INV-001 charge ch_123 card"
            )
            self.assertEqual(statement_data[0]["note"], "Custom Note charge")
            ...
            self.assertEqual(
                statement_data[1]["name"], "stripe fee INV-001 charge ch_123 card",
            )
            self.assertEqual(statement_data[1]["note"], "Fee Note charge")
            ...
            self.assertEqual(
                statement_data[2]["name"], "stripe INV-002 charge ch_456 card"
            )
            self.assertEqual(statement_data[2]["note"], "Custom Note charge")
            ...
            self.assertEqual(
                statement_data[3]["name"], "stripe fee INV-002 charge ch_456 card",
            )
            self.assertEqual(statement_data[3]["note"], "Fee Note charge")
