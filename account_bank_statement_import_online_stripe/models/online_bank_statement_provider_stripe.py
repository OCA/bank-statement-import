...
class OnlineBankStatementProviderStripe(models.Model):
    ...
    def _obtain_statement_data(self, date_since, date_until):
        ...
        date_since_ts = int(date_since.timestamp())
        date_until_ts = int(date_until.timestamp())
        params = [
            ("created[gte]", date_since_ts),
            ("created[lt]", date_until_ts),
        ]
        for s in (self.stripe_expand or "").split(","):
            s = s.strip()
            if not s:
                continue
        ...
        for tx in self._stripe_api_get_all("/balance_transactions", params):
            if tx["currency"].lower() != currency.name.lower():
                continue
            tx_date = datetime.utcfromtimestamp(tx["created"])
            lines.append(
                {
                    "name": _safe_format(self.stripe_label, tx),
                    "note": _safe_format(self.stripe_note, tx),
                    "ref": _safe_format(self.stripe_reference, tx),
            ...
            if tx.get("fee"):
                lines.append(
                    {
                        "name": _safe_format(self.stripe_fee_label, tx),
                        "note": _safe_format(self.stripe_fee_note, tx),
                        "ref": _safe_format(self.stripe_fee_reference, tx),
    ...
    def _stripe_api_get(self, path, params=None, data=None):
        ...
        try:
            ...
            raise UserError(
                "Stripe API call failed: %s: %s" % (path, content)
            ) from e


def _safe_format(template, kwargs):
    """Format a template string using dot-notation keys from a nested dict.

    Unresolvable paths silently become empty strings.
    """
