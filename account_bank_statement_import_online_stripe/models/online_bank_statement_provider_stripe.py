...
class OnlineBankStatementProviderStripe(models.Model):
    ...
    def _obtain_statement_data(self, date_since, date_until):
        ...
        for tx in self._stripe_api_get_all("/balance_transactions", params):
            ...
            lines.append(
                {
                    "name": safe_format(self.stripe_label, tx),
                    "note": safe_format(self.stripe_note, tx),
            ...
            if tx.get("fee"):
                lines.append(
                    {
                        "name": safe_format(self.stripe_fee_label, tx),
                        "note": safe_format(self.stripe_fee_note, tx),
    ...
    def _stripe_api_get(self, path, params=None, data=None):
        ...
        try:
            ...
            raise UserError(
                _("Stripe API call failed: %s: %s") % (path, content)
            ) from e
