...
class PontoInterface(models.AbstractModel):
    ...
    def _ponto_synchronisation(self, access_data):
        ...
        if response.status_code == 400:
            ...
            _logger.debug(_("Synchronization request rejected: %s"), response.text)
            return
        data = self._get_response_data(response)
        ...
        if not sync_id:
            raise UserError(_("Ponto : no synchronization id in data %s") % data)
        ...
        while number < retries:
            ...
            time.sleep(40)
            if number == retries:
                _logger.debug(_("No longer waiting for synchronization"))
    ...
    def _synchronization_done(self, access_data, url):
        ...
        if status not in ("success", "error"):
            return False
        if status != "error":
            _logger.debug(_("Synchronization was succesfully completed"))
        else:
            _logger.debug(_("Synchronization had an error: %s"), response.text)
        return True
