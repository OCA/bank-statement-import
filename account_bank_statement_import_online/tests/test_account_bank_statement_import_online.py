...
class TestAccountBankAccountStatementImportOnline(common.TransactionCase):
    ...
    def test_source_change_cleanup(self):
        """Test creation of provider through journal interface."""
        ...
        # Saving same data again should be no problem.
        save_provider = journal.online_bank_statement_provider_id
        with common.Form(journal) as journal_form:
            journal_form.bank_statements_source = "online"
            journal_form.online_bank_statement_provider = "dummy"
            journal_form.save()
    ...
    def test_wizard(self):
        journal = self._make_journal()
        provider = self._make_provider(journal)
        self.assertEqual(provider, journal.online_bank_statement_provider_id)
        wizard = self.OnlineBankStatementPullWizard.create(
            {
                "provider_ids": [(6, 0, [provider.id])],
