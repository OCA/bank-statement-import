...
class OnlineBankStatementProvider(models.Model):
    ...
    def _mercury_get(self, session, path, params=None):
        ...
        try:
            ...
            raise UserError(
                _("Mercury API request failed: %(error)s") % {"error": str(exc)}
            ) from exc
        if resp.status_code == 401:
            raise UserError(_("Mercury API key is invalid or expired (HTTP 401)."))
        if not resp.ok:
            raise UserError(
                _("Mercury API returned HTTP %(status)s: %(body)s")
    ...
    def _mercury_resolve_account_id(self, session):
        ...
        if not accounts:
            raise UserError(_("No Mercury accounts found for this API key."))
