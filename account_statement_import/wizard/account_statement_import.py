...
class AccountStatementImport(models.TransientModel):
    ...
    statement_file = fields.Binary(
        string='Statement File', required=True,
    ...
    def import_single_file(self, file_data, result):
        ...
        logger.info(
            ...
            self.statement_filename, len(parsing_data))
        i = 0
        for single_statement_data in parsing_data:
            i += 1
            logger.debug('account %d: single_statement_data=%s', i, single_statement_data)
    ...
    def import_single_statement(self, single_statement_data, result):
        ...
        if not journal.default_account_id:
            raise UserError(_(
                "The Bank Accounting Account in not set on the "
                "journal '%s'.") % journal.display_name)
    ...
    def _match_currency(self, currency_code):
        ...
        if not currency:
            raise UserError(_(
                ...
                "but there is no such currency in Odoo.") % currency_code)
        return currency
    ...
    def _complete_stmts_vals(self, stmts_vals, journal, account_number):
        for st_vals in stmts_vals:
            ...
            for lvals in st_vals['transactions']:
                ...
                if unique_import_id:
                    sanitized_account_number = sanitize_account_number(account_number)
                    lvals['unique_import_id'] = (sanitized_account_number and sanitized_account_number + '-' or '') + str(journal.id) + '-' + unique_import_id

                if (
                        not lvals.get('partner_bank_id') and
                        lvals.get('account_number') and
                        not lvals.get('partner_id')):
                    # Find the partner and his bank account or create the bank account. The partner selected during the
                    # reconciliation process will be linked to the bank when the statement is closed.
                    self._update_partner_from_account_number(lvals)
                if not lvals.get('payment_ref'):
                    raise UserError(_("Missing payment_ref")) # TODO
        return stmts_vals
